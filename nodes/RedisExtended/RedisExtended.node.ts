	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			// ----------------------------------
			{
				...
				default: '',
				required: true,
				description: 'Name of the hash key in Redis',
			},
			{
				displayName: 'Field',
				name: 'field',
				...
				default: '',
				required: true,
				description: 'Name of the field to get from the hash',
			},
			{
			...
			},
			{
				...
				required: true,
				description:
					'Field name(s) to delete from the hash. Use comma to separate multiple fields.',
			},
			...
			for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
				try {
					...
					if (operation === 'delete') {
						...
						const hashKey = this.getNodeParameter('key', itemIndex) as string;
						...
						const hashKey = this.getNodeParameter('key', itemIndex) as string;
						const fieldsParam = this.getNodeParameter('fields', itemIndex) as string;
						...
						const deletedCount = await client.hDel(hashKey, fields);

						returnItems.push({ json: { deletedFields: deletedCount } });
					} else if (operation === 'hkeys') {
						const hashKey = this.getNodeParameter('key', itemIndex) as string;
						const propertyName = this.getNodeParameter('propertyName', itemIndex) as string;

						const keys = await client.hKeys(hashKey);

						const options = this.getNodeParameter('options', itemIndex, {});

						if (options.dotNotation === false) {
							item.json[propertyName] = keys;
						} else {
							set(item.json, propertyName, keys);
						}
